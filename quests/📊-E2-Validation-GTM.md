# 📊 E2 — Validation / GTM / Market

**Sphere:** E | **Quest Line:** Testing Whether People Actually Want It

> 📊 *"Most of your assumptions are wrong. The question is: which ones?"*

---

## 🎯 Quest Overview

| | |
|---|---|
| 🏷️ **Sphere** | E — Entrepreneurship |
| 🎯 **Core question** | How do you test demand before building supply? |
| 👤 **Best for** | People who can handle being wrong and want data over opinions |
| 🏆 **Good first output** | Validation checklist or fake landing page test |

---

## 📖 Theory Bite

| Concept | What it means |
|---------|---------------|
| Customer discovery | Talk to 20 potential users before building. Not friends. |
| Validation loop | Hypothesis → test → learn → pivot or persevere |
| GTM channels | Search, social, sales, partnerships, communities — how users find you |
| Messaging hierarchy | Who it's for → what it does → why it's different → proof |

**Daily reality:** Writing interview scripts, running experiments, reading analytics, and accepting that most assumptions are wrong.

---

## ⚔️ Quests

### 🗝️ Quest E2.1 — Validation Checklist
**⏱ Time:** ~30 min | **Difficulty:** ⭐ Beginner

**Quest:** Build 10 questions you need answered before building anything.

**Steps:**
1. Write 10 validation questions for your idea
2. Rank by testability:

| # | Question | Easy to test? (1-5) | How to test |
|---|----------|---------------------|-------------|
| 1 | | | |
| ... | | | |
| 10 | | | |

3. Pick the top 3 — these are your first experiments

---

### 🗝️ Quest E2.2 — Interview Script
**⏱ Time:** ~30 min | **Difficulty:** ⭐⭐ Intermediate

**Quest:** Write 5 customer interview questions that actually work.

**Rules:**
- No "would you use this?" → people lie to be nice
- No "do you think this is a good idea?" → opinions ≠ behavior
- Only "tell me about the last time you..." → reveals real behavior

| # | Your question | What it reveals |
|---|---------------|-----------------|
| 1 | | |
| 2 | | | 
| 3 | | |
| 4 | | |
| 5 | | |

<details>
<summary>� Good vs. Bad interview questions</summary>

| Bad | Why | Good instead |
|-----|-----|-------------|
| "Would you use an app that tracks habits?" | People say yes to hypotheticals | "How do you currently track your habits?" |
| "Do you think this is a good idea?" | Opinions ≠ behavior | "When did you last try to solve this problem?" |
| "How much would you pay?" | Stated preferences ≠ revealed preferences | "What have you already spent money on for this?" |

</details>

---

### 🗝️ Quest E2.3 — GTM Map
**⏱ Time:** ~30 min | **Difficulty:** ⭐⭐ Intermediate

**Quest:** Map how users will discover your product.

```
GTM MAP
├── 👥 Audience: Who are they? (1 sentence)
├── 🎯 Need: What do they want?
├── 💬 Message: What do you say to them?
├── 📢 First channel: Where do you reach them?
└── 📏 Success metric: How do you know it worked?
```

---

### 🗝️ Quest E2.4 — Landing Page Test
**⏱ Time:** ~45 min | **Difficulty:** ⭐⭐ Intermediate

**Quest:** Write copy for a fake landing page. No product exists yet — can you still get signups?

**Steps:**
1. Write:
   - **Headline** (6-10 words, benefit-focused)
   - **Subheadline** (1 sentence expanding the headline)
   - **3 bullet benefits**
   - **One proof point** (testimonial, stat, or demo)
   - **CTA** (what should they do?)

<details>
<summary>� Headline formulas that work</summary>

- "Do [outcome] without [pain point]"
- "The [category] for [specific audience]"  
- "Stop [pain]. Start [benefit]."
- "[Number] ways to [outcome] in [timeframe]"

</details>

---

### 🗝️ Quest E2.5 — Competitive Audit
**⏱ Time:** ~45 min | **Difficulty:** ⭐⭐ Intermediate

**Quest:** Find 3 competitors and identify your differentiation.

| Competitor | What they do well | What they miss | What you'd do differently |
|-----------|------------------|----------------|--------------------------|
| | | | |
| | | | |
| | | | |

**Reflection:** Did you feel discouraged by competition, or energized by the gaps you found?

---

## 🧭 Test-Drive Verdict

| 📡 Signal | 🚩 Flag |
|--------|------|
| 🐇 You wanted to actually run the experiments | 🟢 **Green** |
| ✅ Being wrong felt like progress, not failure | 🟢 **Green** |
| 🔨 You preferred building to testing | 🟡 **Yellow** |
| 😰 The uncertainty made you anxious | 🟡 **Yellow** |
| ❌ You couldn't handle the "we were wrong" cycle | 🔴 **Red** |

---

## 📚 Resources

| Resource | Type | Best for |
|----------|------|----------|
| The Mom Test (Fitzpatrick) | Book | Customer interview technique |
| Running Lean (Maurya) | Book | Validation loop methodology |
| Landing Page templates | Tool | Carrd.co, Framer, Webflow |
| Google Trends | Tool | Testing demand signals |

---

## 🔗 Connections

- **Found a problem?** → 💼 E1 — Venture / Product
- **Need partners to reach market?** → 🤝 E3 — Ecosystem
- **Ready to operate like a founder?** → 📋 E4 — Founder Ops

---

## 📋 Progress Tracker

| Quest | Time | Status |
|-------|------|--------|
| E2.1 — Hypothesis List | ~20 min | ⬜ |
| E2.2 — Interview Script | ~30 min | ⬜ |
| E2.3 — Landing Page | ~45 min | ⬜ |
| E2.4 — Pricing Experiment | ~30 min | ⬜ |
| E2.5 — Competitive Audit | ~30 min | ⬜ |

**Total estimated time: ~2.5 hours**

Mark completed quests with ✅ in your fork.

---

## 🎯 The Big Picture

Complete all quests and your validation map will evolve:

| Stage | Unlocked By |
|-------|-------------|
| 📝 Hypothesis Writer | Quest E2.1 |
| 🗣️ Interview Ready | Quest E2.2 |
| 🌐 Landing Page Live | Quest E2.3 |
| 💰 Pricing Experimenter | Quest E2.4 |
| 🔍 Competitive Analyst | Quest E2.5 |
| 🏆 Validation-Ready | All Quests Complete |

**You started assuming your idea was good. You ended with evidence — and the habit of testing before building.** 🎉
